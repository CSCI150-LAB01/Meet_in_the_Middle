...
export async function POST(request: Request) {
	...
	if (!data.meetingDateTime || !data.title || !data.placeId) {
		return NextResponse.json(
			{ message: 'Must include meetingDateTime, title, and placeId' },
			{ status: 400 },
		);
	}
...
export async function PATCH(request: Request) {
	...
	if (!data.meetingDateTime && !data.title && !data.placeId) {
		return NextResponse.json(
			{
				message:
					'Must send at least one of the following fields meetingDateTime, placeId, title',
			},
			{ status: 400 },
		);
	}
...
export async function DELETE(request: Request) {
	...
	return NextResponse.json(
		{ message: `deleted meeting ${data.meetingId}` },
		{ status: 200 },
	);
}
