...
export async function DELETE(request: Request) {
    ...
    if (userId === friendId) {
        return NextResponse.json({ message: "Cannot remove self as friend" }, { status: 400 });
    }
    ...
    try {
        ...
        return NextResponse.json({ message: "Error deleting friend relation", error }, { status: 500 });
    }

    return NextResponse.json({ message: "Successfully removed friend", friendId }, { status: 200 });
