...
export async function getData(request: Request) {
	try {
		...
		return NextResponse.json(
			{ message: 'Internal Error, request.json()' },
			{ status: 500 },
		);
	}
}
...
export async function getDefaultLocationById(defaultLocationId: string) {
	...
	try {
		...
		if (error instanceof mongoose.Error.CastError) {
			...
			return NextResponse.json(
				{ message: 'Default Location Not Found', error },
				{ status: 404 },
			);
		}
		console.log(error.stack);
		return NextResponse.json(
			{ message: 'Error retrieving default location by Id' },
			{ status: 500 },
		);
	}
	if (!defaultLocation) {
...
export async function getMeetingById(meetingId: string) {
	...
	try {
		...
		if (error instanceof mongoose.Error.CastError) {
			...
			return NextResponse.json(
				{ message: 'Meetings Not Found', error },
				{ status: 404 },
			);
		}
		console.log(error.stack);
		return NextResponse.json(
			{ message: 'Error retrieving Meetings by Id' },
			{ status: 500 },
		);
	}
	if (!meeting) {
